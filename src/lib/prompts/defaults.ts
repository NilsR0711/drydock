...
export const DEFAULT_TEMPLATES: Record<TemplateName, string> = {
  ...
  release: [
    ...
    "",
    "Goal: perform THIS repository's release end-to-end.",
    "",
    "## Release playbook",
    "",
    "$RELEASE_PLAYBOOK",
    "",
    "1. Establish how this repo releases. If a playbook is recorded above, follow it",
    "   and verify each step still matches reality (the workflow still exists, the",
    "   scripts are unchanged); only re-investigate a step that no longer fits. If no",
    "   playbook is recorded, investigate from scratch: read the CI workflows",
    "   (.github/workflows), package.json scripts, release-please / changesets /",
    "   semantic-release config, CHANGELOG, and the prior tags (`git tag`,",
    "   `gh release list`). Determine how a release is normally cut here — e.g. a",
    "   release-please `workflow_dispatch`, an `npm publish`, a tag + GitHub Release,",
    "   or a changelog + tag convention.",
    ...
    "",
    "When done, write TWO files (do not commit either — Drydock reads and removes them):",
    "",
    "- `.drydock/RELEASE.md`: in either form — start with a `Tag: <tag>` line (e.g.",
    "  `Tag: v1.4.0`) followed by the release title, OR make the first line a",
    "  version-looking title (e.g. `v1.4.0`) that doubles as the tag. Then a blank",
    "  line, then notes on what you did.",
    "- `.drydock/RELEASE_PLAYBOOK.md`: the corrected, up-to-date, step-by-step",
    "  procedure for releasing THIS repo, so the next run can follow it without",
    "  re-investigating. Record the exact commands/steps you actually used (in order),",
    "  including the version-bump and verification steps. NEVER include secrets, tokens,",
    "  or credentials — steps and commands only. If a playbook was recorded above and",
    "  it was correct, write it back (refined); if a step had drifted, write the fixed",
    "  version.",
    "",
