...
export const DEFAULT_TEMPLATES: Record<TemplateName, string> = {
  ...
  release: [
    ...
    "   the GitHub Release, or the published package) before you finish.",
    "",
    "When done, write a short `.drydock/RELEASE.md`: the first line is the release",
    "title (the tag, e.g. `v1.4.0`, when you cut one directly), then a blank line,",
    "then notes on what you did. Add a `Tag: <tag>` line if a tag was created. Do not",
    "commit this file — Drydock reads it to record the run, then removes it.",
    "",
