...
interface WorktreeApi {
  ...
  /** Check out an existing (pushed) branch, used to resume a job on its prior work (issue #257). */
...
async function runJobCore(jobId: number, deps: RunJobDeps, send: NotifyEvent): Promise<Job> {
  ...
  try {
    ...
    wt = resumeOnExistingBranch
      ? await worktrees.prepareForBranch(repo, job.branch as string, `job-${job.id}`)
