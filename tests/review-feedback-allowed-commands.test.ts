...
// without spawning a real agent. This is the seam that captures the spawn
// options, where `repo.allowedCommands` must be threaded (issue #329).
...
describe("review-feedback side session — allowedCommands wiring (issue #329)", () => {
  it("forwards repo.allowedCommands to the spawned side session", async () => {
    const repo = addRepo(
      {
        path: "/r",
        name: "r",
    ...
    expect(spawnDeps()?.allowedCommands).toEqual(["git", "xcodebuild"]);
    // Must stay a side session: a normal spawn would throw an invalid transition.
    expect(spawnDeps()?.sideSession).toBe(true);
  });

  it("forwards an empty allowlist when the repo has none configured", async () => {
    ...
    expect(spawnSpy).toHaveBeenCalledTimes(1);
    expect(spawnDeps()?.allowedCommands).toEqual([]);
  });
});

/** The spawn options (4th positional arg) from the single recorded spawn call. */
function spawnDeps(): { allowedCommands?: string[]; sideSession?: boolean } | undefined {
  return spawnSpy.mock.calls[0]?.[3] as
    | { allowedCommands?: string[]; sideSession?: boolean }
    | undefined;
}
